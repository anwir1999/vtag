...
void MQTT_DevConf_Payload_Convert(char *str, int16_t ss, uint8_t cc, char *type, int16_t rq, uint16_t p, uint8_t m, long ts, char *version, uint8_t bat_level, char *cn, uint8_t n, int _ss, int WM)
{
	memset(str, 0, MQTT_TX_Str_Buf_Lenght);
	sprintf(str,"{\"ss\":%d,\"CC\":%d,\"Type\":\"%s\",\"r\":%d,\"MMC\":{\"P\":%d,\"M\":%d},\"T\":%ld,\"V\":\"%s\",\"B\":%d,\"Cn\":\"%s\",\"N\":%d,\"_ss\":%d,\"WM\":%d,\"RR\":%d%d}", ss, cc, type, rq, p, m, ts, version, bat_level, cn, n, _ss, WM, Reboot_reason, Backup_reason);
}
